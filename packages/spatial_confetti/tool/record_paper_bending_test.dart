...
import '../test/paper_bending_test.dart'
    show flexiblePaper, loadPaper, bendingDepth, sheetNormal;
...
void main() {
  ...
  test('record force-driven paper bending and recovery', () async {
          ...
          'vertices': [
            for (final v in p.surfaceVertices)
              [
                (v - p.position).dot(r),
                (v - p.position).dot(up),
                (v - p.position).dot(normal),
              ],
          ],
        });
        if ([0, 6, 9, 30, 69].contains(frame)) {
          const size = ui.Size(320, 320),
              camera = ConfettiCamera(viewHeight: .24);
          ...
          final picture = recorder.endRecording(),
              image = await picture.toImage(320, 320);
          final bytes = (await image.toByteData(
            format: ui.ImageByteFormat.png,
          ))!.buffer.asUint8List();
          File('doc/assets/paper-$name-$frame.png').writeAsBytesSync(bytes);
          image.dispose();
          picture.dispose();
        }
      }
    ...
    File('doc/assets/paper-bending.json').writeAsStringSync(jsonEncode(rows));
  });
}
