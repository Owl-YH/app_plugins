...
void main() {
  ...
  test(
    ...
    () async {
      ...
      final assets = Directory('docs/assets')..createSync(recursive: true);
      const selected = String.fromEnvironment('GUIDE_SCENES');
      final requested = selected.split(',').where((id) => id.isNotEmpty).toSet();
      final existingFile = File('${assets.path}/capture.json');
      final existing = requested.isNotEmpty && existingFile.existsSync()
          ? (jsonDecode(existingFile.readAsStringSync()) as List).cast<Map<String, dynamic>>()
          : <Map<String, dynamic>>[];
      final report = <Map<String, Object>>[];
      for (final scene in guideScenes(size)) {
        if (requested.isNotEmpty && !requested.contains(scene.id)) {
          report.add(
            Map<String, Object>.from(existing.singleWhere((row) => row['id'] == scene.id)),
          );
          continue;
        }
        final directory = Directory('${root.path}/${scene.id}')..createSync(recursive: true);
        ...
        for (
          var frame = 0;
          frame <= (scene.duration.inMicroseconds * fps / Duration.microsecondsPerSecond).round();
          frame++
        ) {
          ...
          File('${directory.path}/${frame.toString().padLeft(4, '0')}.png').writeAsBytesSync(data);
          if (frame ==
              (scene.posterTime.inMicroseconds * fps / Duration.microsecondsPerSecond).round())
            File('${assets.path}/${scene.id}.png').writeAsBytesSync(data);
          image.dispose();
          picture.dispose();
          if (sim.stats.particles > peakParticles) peakParticles = sim.stats.particles;
          if (painter.stats.streaks > peakStreaks) peakStreaks = painter.stats.streaks;
        }
        ...
        report.add({
          'id': scene.id,
          'frames':
              (scene.duration.inMicroseconds * fps / Duration.microsecondsPerSecond).round() + 1,
          'fps': fps,
